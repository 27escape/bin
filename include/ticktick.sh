#!/usr/bin/env bash
# taken from https://github.com/kristopolous/TickTick
# should be included in files early on to ensure the functions are available
...
# kpm this was mine, not needed if using . as a field separator
# tickItem() {
#     # echo "@ Line `caller | sed s/\ NULL//`:"
#     item=`echo "$1" | sed s:/:_:`

#     set | grep __tick_data_$item |grep -v BASH_EXECUTION_STRING | sed s/^__tick_data_$item=//
# }
